...
public class ScanView extends Activity implements OnClickListener {
    ...
    private EditText txtDevAddr;

    private Button btSetAddr;
    private Button btGetAddr;
    ...
    private void initView() {
        ...
        txtDevAddr = (EditText) findViewById(R.id.txt_addr);
        btSetAddr = (Button) findViewById(R.id.btsetaddr);
        btGetAddr = (Button) findViewById(R.id.btgetaddr);
        ...
        btGetAddr.setOnClickListener(this);
    ...
    protected void onResume() {
        ...
        if (!started) {
            Ant8.setChecked(true);
            btSetAntenna.performClick();
            txtDevAddr.setText(Reader.rrlib.GetInventoryPatameter().ComAddr + "");
    ...
    public void onClick(View view) {
        try {
            if (view == btSetFre) {
                ...
                mThread.start();
            } else if (view == btSetAddr) {
                final byte ComAddr = (byte) (int) Integer.valueOf(txtDevAddr.getText().toString());
                Thread mThread = new Thread(new Runnable() {
                    @Override
                    public void run() {
                        String temp = "";
                        int result = Reader.rrlib.SetAddress(ComAddr);
                        if (result != 0)
                            temp = getString(R.string.set_failed);
                        else
                            temp = getString(R.string.set_success);
                ...
                mThread.start();
            } else if (view == btGetAddr) {
                byte ComAddr = Reader.rrlib.GetInventoryPatameter().ComAddr;
                txtDevAddr.setText(ComAddr + "");
            }
        } catch (Exception ex) {
        }
    }
