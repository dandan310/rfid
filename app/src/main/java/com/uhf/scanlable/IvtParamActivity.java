...
public class IvtParamActivity extends Activity implements OnClickListener{
	...
	public void onClick(View arg0) {
		...
		if(arg0 == btSet)
		{
			...
			else if(param.Session==5)param.Session=254;
			else if(param.Session==6)param.Session=253;
	...
	private void GetParam()
	{
		...
		else if(param.Session==254)
		{
			spS.setSelection(5, false);
		}
		else if(param.Session==253)
		{
			spS.setSelection(6, false);
		}
		else
			spS.setSelection(param.Session, false);
		spQ.setSelection(param.QValue & 0x0F, false);
